...
EMO_SIZE = 4
TRAN_SIZE = 4
SAMPLE_SIZE = 1 * ((PERSON_SIZE * EMO_SIZE - 8) * TRAN_SIZE - 2)
# SAMPLE_SIZE = 1 * (PERSON_SIZE * EMO_SIZE * TRAN_SIZE)
...
def load():
  ...
  with h5py.File(SOURCE_FOLDER_PREFIX + 'all_info_large_'+SESSION+'_'+str(EMO_SIZE)+'emo.h5', 'w') as f:
  ...
  with open(SOURCE_FOLDER_PREFIX + 'all_info_large_'
                +SESSION+'_'+str(EMO_SIZE)+'emo.txt', 'w') as f:
    f.write(SOURCE_FOLDER_PREFIX + 'all_info_large_'
            +SESSION+'_'+str(EMO_SIZE)+'emo.h5' + "\n")
