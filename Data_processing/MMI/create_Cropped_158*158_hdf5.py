...
IMAGE_SIZE = 158
TARGET_SIZE = 158
WORK_DIRECTORY = '../../MMI_dataset/MMI_CROPPED/'
NUM_CHANNELS = 3
PIXEL_DEPTH = 255.0
EXCLUTION = [
...
def load():
  ...
  data, labels = extract_data(WORK_DIRECTORY, 28)

  train_data, train_labels = augmentImage(
    data, labels)
  print data.shape

  with h5py.File(SOURCE_FOLDER_PREFIX + 'all_info_large_cropped.h5', 'w') as f:
    f['data'] = train_data / 255
    f['person'] = toOneHot(train_labels[:,0], 7)
    f['personclass'] = train_labels[:,0]
    f['emotion'] = toOneHot(train_labels[:,1], 4)
    f['emotionclass'] = train_labels[:,1]
    f['session'] = toOneHot(numpy.zeros((SAMPLE_SIZE,1),dtype=numpy.int8), 2)
    f['sessionclass'] = numpy.zeros((SAMPLE_SIZE,1),dtype=numpy.int8)
    f['transform'] = toOneHot(train_labels[:,2], 4)
    f['transformclass'] = train_labels[:,2]

  with open(SOURCE_FOLDER_PREFIX + 'all_info_large_cropped.txt', 'w') as f:
    f.write("../../Source/MMI/" + 'all_info_large_cropped.h5' + "\n")
