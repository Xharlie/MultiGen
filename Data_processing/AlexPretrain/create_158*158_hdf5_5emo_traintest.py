...
IMAGE_SIZE = 158
TARGET_SIZE = 158
WORK_DIRECTORY = '../../KDEF_dataset/KDEF_CROPPED_5emo/'
...
def load():
  ...
  data, labels = extract_data(WORK_DIRECTORY, 650)

  train_data, train_labels = augmentImage(
    data, labels)
  print train_data.shape

  with h5py.File(SOURCE_FOLDER_PREFIX + 'all_info_large_5emo_'+SESSION+'Session.h5', 'w') as f:
  ...
  with open(SOURCE_FOLDER_PREFIX + 'all_info_large_5emo_'+SESSION+'Session.txt', 'w') as f:
    f.write("../../Source/AlexPretrain/" + 'all_info_large_5emo_'+SESSION+'Session.h5' + "\n")
...
def modified():
  ...
  person_index = []
  emotion_dict = {'0':'0','1':'1','2':'2','5':'3','6':'4'}
  counter = 0
  for i in range(0, 70):
  ...
  for img in imgList:
    comp = img.split('/')
    path = comp[0] + '/' + comp[1] + '/' + comp[2] + '/KDEF_CROPPED_5emo/'
    sub =comp[4].split('-')
